...
!
  ...
  real :: Omega_int=0.,Omega_ext=0.
  real :: xexp_diffrot=1.,kx_diffrot=1.,kz_diffrot=0., phase_diffrot=0.
...
!
  namelist /hydro_run_pars/ &
      ...
      ldamp_fade, tfade_start, lOmega_int, Omega_int, Omega_ext, lupw_uu, othresh, &
...
!
        if (llevitation_experiment_in_a_box) then
          if (lOmega_int) then
            print*, 'lOmega_int probably not compatible with levitation-experiment-in-a-box :) (: :) (: :o'
            call fatal_error("udamping",":o")
          endif
!
!  external angular velocity, uref=(y,-x,0)*Omega_ext, and
!  calculate work done to sustain uniform rotation on outer cylinder/sphere
!
          pdamp = step(p%rcyl_mn,rdampext,wdamp) ! outer damping profile
!
          fext(:,1)=-dampuext*pdamp*(f(l1:l2,m,n,iux)-y(m)*Omega_ext)
          fext(:,2)=-dampuext*pdamp*(f(l1:l2,m,n,iuy)+x(l1:l2)*Omega_ext)
          fext(:,3)=-dampuext*pdamp*(f(l1:l2,m,n,iuz))
          df(l1:l2,m,n,iux)=df(l1:l2,m,n,iux)+fext(:,1)
          df(l1:l2,m,n,iuy)=df(l1:l2,m,n,iuy)+fext(:,2)
          df(l1:l2,m,n,iuz)=df(l1:l2,m,n,iuz)+fext(:,3)
!
          if (idiag_fextm/=0) then
            fext_work=f(l1:l2,m,n,iux)*fext(:,1)&
                     +f(l1:l2,m,n,iuy)*fext(:,2)&
